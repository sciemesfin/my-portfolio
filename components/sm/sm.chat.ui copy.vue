                  ...
                  <div class="flex flex-col">
                    <div v-for="(item, i) in chats" :key="i">
                      ...
                      <div class="flex flex-row p-2">
                        <div class="py-2 flex self-start">
                        ...
                        <div
                          v-if="(item.a === '_' || loading) && i===chats.length-1"
                          class="w-full three-dot px-10"
                        ></div>
                        <div v-else class="w-full p-2">
                          <div
                            class="bg-gray-800 p-3 md:py-5 md:px-16 rounded-bl-xl rounded-tr-xl rounded-br-xl mb-2 relative"
                          >
                            <code
                              class="!whitespace-pre-wrap hljs language-php-template"
                              ><span class="xml">
                                <span class="hljs-tag"
                                  ><span
                                    class="hljs-name text-sm text-white"
                                    ref="message"
                                  >
                                    {{ item.a }}
                                  </span>
...
export default {
  setup() {
    ...
    const getData = async () => {
      chats.value.push({ q: inputValue.value, a: "_" });
      loading.value = true;
      error.value = null;
      inputValue.value = "";
      try {
        ...
        const res = await fetch("https://api.openai.com/v1/completions", {
          ...
          body: JSON.stringify({
            prompt: chats.value[chats.value.length - 1].q,
        ...
        // chats.value=[{ q: inputValue.value, a: data.choices[0].text }];
        // const oldChats=chats

        chats.value = chats.value.map((x) => {
          return { ...x, a: x.a === "_" ? data.choices[0].text : x.a };
        });
        // chats.push(a)
      } catch (err) {
        error.value = err;
